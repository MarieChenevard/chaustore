...
if(!empty($_POST ['add_size'])){
  $answer= $_POST['add_size'];
  $bdd->exec("INSERT INTO `size`(name)VALUES('$answer')");
  // var_dump($answer);
}
...
<div class="listing">
  <form method="POST" action="add_size.php">
    <input class="write" type="text" name="add_size" id="add_size" value="">
    <input class="add" type="submit" name="submit "value="Ajouter !">
  </form>

<?php
$reponse = $bdd->query('SELECT `name` FROM size');
